...
#define lc v * 2
    ...
    void push(int v, int tl, int tr)
    {
        if (is_lazy[v])
        {
            tree[v].maxi = max(tree[v].maxi, lazy[v]); // Lazy push krbo , so update curr node

            if (tl != tr)
            {
                lazy[lc] = max(lazy[lc], lazy[v]);
                lazy[rc] = max(lazy[rc], lazy[v]);
    ...
    void update(int v, int tl, int tr, int l, int r, int add)
    {
        ...
        if (l <= tl && tr <= r)
        {
            ...
            lazy[v] = max(lazy[v], add); // Update Lazy
            is_lazy[v] = true;
