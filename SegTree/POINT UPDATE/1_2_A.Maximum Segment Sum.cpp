...
struct SegTree
{
    ...
    node merge(node seg1, node seg2)
    {
        node segment(0);

        segment.seg = max({seg1.seg, seg2.seg, seg1.suf + seg2.pre});
        segment.suf = max({seg2.suf, seg2.sum + seg1.suf});
        segment.pre = max(seg1.pre, seg1.sum + seg2.pre);
        segment.sum = (seg1.sum + seg2.sum);
