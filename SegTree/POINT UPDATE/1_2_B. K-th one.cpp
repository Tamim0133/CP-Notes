...
typedef priority_queue<int> pqll;
...
// ll inv(ll a){return powerMod(a, mod - 2, mod);}
...
// int divideMod(int a, int b){return ((a % mod) * inv(b) % mod) % mod;}

// vll fact(2e5 + 10, 1);

// int nCr(int n, int r){int lob = fact[n];int hor = (fact[r] % mod * fact[n - r] % mod) % mod;int ans = (lob % mod * inv(hor) % mod) % mod;return ans;}
...
struct SegTree
{
    ...
    // merge function (can change to min/max/gcd etc.)
    ...
    int query(int v, int k, int l, int r)
    {
        ...
        int leftChild = tree[v * 2];
        int rightChild = tree[v * 2 + 1];

        int m = (l + r) / 2;

        if (leftChild < k) // right e jaabo with k - leftChild
        {
            return query(2 * v + 1, k - leftChild, m + 1, r);
        }
        else if (leftChild >= k) // left ei ashbo
        {
...
void D_One()
{
    ...
    // cout << st.query(0, 2) << "\n";  // both l and r are 0-based

    // st.update(1, 10);// idx is 0-based
...
int32_t main()
{
    ...
    // Precalculation
    // for (int i = 1; i <= 2e5; i++)  fact[i] = (fact[i - 1] % mod * i % mod) % mod;
